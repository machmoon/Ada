...
<section class="panel" data-kind={kind}>
  {#if kind === 'no-api-key'}
    ...
    <h1 class="title">Silkscreen has no Gemini key yet</h1>
    <p class="lead">
    ...
    <button type="button" class="secondary" onclick={ondismiss}>Back to the form</button>

  {:else if kind === 'timeout'}
    <h1 class="title">The run ran too long</h1>
    <p class="lead">
  ...
  {:else if kind === 'network'}
    <h1 class="title">The service is not running</h1>
    <p class="lead">
  ...
  {:else if kind === 'validation'}
    <h1 class="title">The request was rejected</h1>
    <p class="lead">{error.message}</p>
    <button type="button" class="primary" onclick={ondismiss}>Edit the request</button>

  {:else if kind === 'too-large'}
    <h1 class="title">The request is too large</h1>
    <p class="lead">{error.message}</p>
    <button type="button" class="primary" onclick={ondismiss}>Edit the request</button>

  {:else if kind === 'not-found'}
    <h1 class="title">No such endpoint</h1>
    <p class="lead">
      {error.message} The bundle and the service are probably different versions of the app.
    </p>
    <button type="button" class="primary" onclick={ondismiss}>Back to the form</button>

  {:else if kind === 'internal'}
    <h1 class="title">The service hit an internal error</h1>
    <p class="lead">
    ...
    {#if error.errorId}<div class="mono errid">{error.errorId}</div>{/if}
    <button type="button" class="primary" onclick={onretry}>Try again</button>

  {:else}
    <h1 class="title">The model provider failed</h1>
    <p class="lead">{error.message}</p>
    <button type="button" class="primary" onclick={onretry}>Try again</button>
  {/if}
</section>
