...
```
319 passed — no network, no API key, no KiCad install
```
...
| Component | State |
|---|---|
| `kicad.py` — `.kicad_pcb` read/write | **Working** · 21 tests |
...
| Testable without KiCad | No | **Yes, all 319 tests** |
...
engine/
  ...
  tests/          319 tests — no network, no API keys, no KiCad
...
**1. Test suite** — 319 tests, no warnings (four key-gated live-model tests skip unless `GOOGLE_API_KEY` is set):

```
319 passed in 190.85s
```
