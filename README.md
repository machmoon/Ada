...
```
341 passed — no network, no API key, no KiCad install
```
...
| `agents/` — datasheet, propose, review, pipeline | **Working** · 28 tests |
...
| `service/` — Cloud Run + Firestore cache | **Working** · 79 tests |
...
| Testable without KiCad | No | **Yes, all 341 tests** |
...
engine/
  ...
  tests/          341 tests — no network, no API keys, no KiCad
...
**1. Test suite** — 341 tests, no warnings (four key-gated live-model tests skip unless `GOOGLE_API_KEY` is set):

```
341 passed in 190.85s
```
