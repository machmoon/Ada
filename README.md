...
```
296 passed — no network, no API key, no KiCad install
```
...
| `service/` — Cloud Run + Firestore cache | **Working** · 49 tests |
...
| Testable without KiCad | No | **Yes, all 296 tests** |
...
engine/
  ...
  tests/          296 tests — no network, no API keys, no KiCad
...
**1. Test suite** — 296 tests, no warnings. The two skips are live-API tests,
which run only when `GOOGLE_API_KEY` is set:

```
294 passed, 2 skipped in 167.23s
```
