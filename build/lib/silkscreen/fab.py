...
def excellon_drill(board: BoardResult) -> str:
    """Every via in the board, as an Excellon drill program.

    Footprints here are all SMD, so the only holes are the vias the router
    placed to change layers. A board with none produces a well-formed empty
    program rather than no file at all -- fabs expect the file, and an empty
    one is the honest answer. What would be dangerous is the reverse: claiming
    no holes on a board that has vias, which leaves them undrilled and the two
    layers unconnected.
