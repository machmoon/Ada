...
**Pre-build net and routing contract. [built]**
The prompt first becomes proposed interface, net, routing, and mechanical constraints.
The engineer approves exact net names and limits before generation. I2C pull-ups and
rise time, route completion, layers, vias, width, length, skew, power drop, board
outline, keepouts, fixed parts, mounting holes, and identifiable thermal spacing are
checked deterministically. A hard rule that is violated or cannot yet be verified
blocks promotion. Soft terms such as shorter traces and connector access only rank
boards after the hard gate passes. Controlled impedance, plane continuity, stubs,
guards, and component height are reported as unresolved until the necessary physical
evidence exists.
