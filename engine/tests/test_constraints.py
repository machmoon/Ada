...
def test_manifest_round_trips_and_enters_the_agent_prompt():
    ...
    assert "Soft preferences rank only designs" in manifest.prompt_block()
...
def test_i2c_pullups_and_rise_time_are_deterministically_verified():
    ...
    assert receipt["hard_gate"] == "passed"
...
def test_power_voltage_drop_uses_routed_copper_geometry():
    raw = _i2c_manifest(
        kind="power",
        name="Power",
        pullups_required=False,
    ...
    assert drop["status"] == "violated"
    assert drop["evidence"]["measured_v"]["SDA"] > 0.01


def test_thermal_separation_is_measured_between_identifiable_power_parts():
    ...
    assert thermal["evidence"]["measured_mm"] == pytest.approx(2)
